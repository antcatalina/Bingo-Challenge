 ...
 */
 ...
 function checkForBingo (bingoCard, drawnNumbers) {
  ...
  var bingo = [
    ...
    drawnNumbers.push("FREE");

    var testArr = [[], [], [], [], [], [], [], [], [], [], [], []];

    for (let i=0; i<12; i++) {
      for (let j=0; j<5; j++) {
        testArr[i].push(bingoCard[bingo[i][j]]);
      }
    }    

    for (let i=0; i<testArr.length; i++) {
      if (drawnNumbers.includes(testArr[i][0]) && 
          drawnNumbers.includes(testArr[i][2]) &&
          drawnNumbers.includes(testArr[i][3]) &&
          drawnNumbers.includes(testArr[i][4])) {
